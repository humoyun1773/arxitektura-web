    ...
    <div className="pt-28 pb-20 min-h-screen bg-white">
      <div className="app-container space-y-16">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-3">
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold font-heading text-slate-900">Byuromizning Professional Arxitektorlari</h1>
          <p className="text-sm sm:text-base text-slate-600">20+ nafar sertifikatlangan mutaxassis har bir loyihada buyurtmachi manfaatini birinchi o'ringa qo'yadi.</p>
        </div>
            ...
            <div key={arch.id} className="bg-white rounded-3xl border border-slate-200 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden group">
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="flex items-center gap-4 text-xs">
                  <div className="flex items-center gap-1.5 text-slate-700">
                    ...
                    <span><strong>{arch.projectsCompleted}</strong> loyiha</span>
                  </div>
                  <div className="flex items-center gap-1.5 text-slate-700">
                    <Award className="w-3.5 h-3.5 text-slate-400" />
                    <span><strong>{arch.experienceYears}</strong> yil tajriba</span>
                  </div>
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button onClick={() => openLeadModal()} className="flex-1 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs transition-colors flex items-center justify-center gap-1.5 cursor-pointer">
                    Bog'lanish <ArrowRight className="w-3.5 h-3.5" />
                  </button>
                </div>
              </div>
              ...
              {activeId === arch.id && (
                <div className="px-6 pb-6 space-y-4 border-t border-slate-100 pt-4">
                  <div className="text-xs font-bold uppercase tracking-wide text-slate-500">Mutaxassislik yo'nalishlari:</div>
                  ...
                  {arch.awards?.[0] && (
                    <div>
                      <div className="text-xs font-bold uppercase tracking-wide text-slate-500 mb-1">Ta'lim:</div>
                      <div className="text-xs text-slate-700">{arch.awards?.[0]}</div>
                    </div>
                  )}
                  {arch.awards && arch.awards.length > 0 && (
                    <div>
                      <div className="text-xs font-bold uppercase tracking-wide text-slate-500 mb-1">Tillar:</div>
        ...
        <div className="bg-slate-900 rounded-3xl p-8 sm:p-12 text-center text-white space-y-5">
          <h3 className="text-2xl sm:text-3xl font-extrabold font-heading">O'z Loyihangiz Uchun Mutaxassis Toping</h3>
          <p className="text-sm text-slate-400 max-w-lg mx-auto">Biz bilan bog'laning — loyihangizga eng mos arxitektor va muhandis jamoasini tashkil qilamiz.</p>
          <button onClick={() => openLeadModal()} className="px-8 py-3.5 bg-white hover:bg-slate-100 text-slate-900 font-bold text-sm rounded-2xl shadow-lg inline-flex items-center gap-2 transition-colors cursor-pointer">
            Bepul Konsultatsiya <ArrowRight className="w-4 h-4" />
          </button>
        </div>
