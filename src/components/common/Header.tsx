...
export const Header: React.FC<HeaderProps> = ({ currentPage, onNavigate }) => {
  ...
  return (
    <>
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 z-50 bg-slate-950 flex flex-col justify-between animate-in fade-in duration-200">
          {/* Top Bar inside Menu */}
          <div className="flex items-center justify-between p-4 sm:p-5 border-b border-slate-800/90 bg-slate-950">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5 bg-slate-950">
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-3.5 px-4 py-3.5 rounded-2xl text-base font-semibold transition-all ${
                    currentPage === link.id
                      ...
                      : 'text-slate-200 bg-slate-900/40 hover:bg-slate-900 border border-slate-800/50'
                  }`}
                >
            ...
            {/* Markaz Info Box */}
            <div className="p-4 rounded-2xl bg-gradient-to-br from-indigo-950/40 to-slate-900/60 border border-indigo-500/20 text-xs space-y-2 text-slate-400">
